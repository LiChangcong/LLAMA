...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if (gestureRecognizer == _singleTapGuesture)
        return YES;
...
- (void)singleTap:(UITapGestureRecognizer *)sender
{
    ...
    BOOL hasFound = [_textContainer enumerateRunRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<TYTextStorageProtocol> textStorage){
        if (_delegateFlags.textStorageClickedAtPoint) {
    ...
    if (!hasFound) {
        if (_delegate && [_delegate respondsToSelector:@selector(attributedLabel:labelSingleTappedWithState:atPoint:)]) {
            [_delegate attributedLabel:self labelSingleTappedWithState:sender.state atPoint:point];
        }
    }
}
...
- (void)longPress:(UILongPressGestureRecognizer *)sender
{
    ...
    if (!hasFound) {
        if (_delegate && [_delegate respondsToSelector:@selector(attributedLabel:labelLongPressedWithState:atPoint:)]) {
            [_delegate attributedLabel:self labelLongPressedWithState:sender.state atPoint:point];
        }
    }
}
