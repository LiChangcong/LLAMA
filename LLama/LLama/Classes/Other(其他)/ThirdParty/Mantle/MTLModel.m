...
//        }
        ...
        if (![obj validateValue:&validatedValue forKey:key error:error]) {
            ...
            if ([propertyType hasPrefix:@"T@"]) {
                ...
                NSString *classString = [propertyType substringWithRange:NSMakeRange(3, [propertyType rangeOfString:@","].location - 4)];
                
                Class classType =  NSClassFromString(classString);
                if ([classType isKindOfClass:[NSString class]] && ([value isKindOfClass:[NSNumber class]] || [value isKindOfClass:[NSValue class]])) {
                    [obj setValue:[value stringValue] forKey:key];
                }
            ...
            else if ([propertyType hasPrefix:@"Ti"])
            {
                ...
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value integerValue]) forKey:key];
                }
            ...
            else if ([propertyType hasPrefix:@"TF"])
            {
                //float
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value floatValue]) forKey:key];
                }
            }
            else if([propertyType hasPrefix:@"Td"]) {
                //double
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value doubleValue]) forKey:key];
                }
            }
            else if([propertyType hasPrefix:@"Tl"])
            {
                //long
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value longValue]) forKey:key];
                }
            }
            else if ([propertyType hasPrefix:@"Tc"]) {
                //char or bool
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value charValue]) forKey:key];
                }
            }
            else if([propertyType hasPrefix:@"Ts"])
            {
                //short
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value shortValue]) forKey:key];
                }
            }else if([propertyType hasPrefix:@"Tq"]) {
                //long long
                if ([value isKindOfClass:[NSString class]]) {
                    [obj setValue:@([value longLongValue]) forKey:key];
                }
